%% This demo is used to reconstruct the HR969 data using the HERO, MF, and SVD algorithms.
...
parfor (iter = 1:length_block,2)
    fprintf('parfor Iteration: %d\n',iter)

    if (left+iter)>=128 && (left+iter)<=185
        continue;
    end
...
for iter2 = 1:512
    ...
    if iter2>=103 && iter2<=127
        continue;
    end

    if iter2>=186 && iter2<=370
        continue;
    end
...
% save('Hnoesy_4D_recon_MF.mat','FID_Rec')